...
from permissions.models import GroupModel
...
@register.simple_tag
def get_version():
    return '%s-%s' % (x_version, timezone.datetime.now().strftime('%Y%d%m%H%M'))


@register.simple_tag
def get_app_info():
    return format_table({"version": x_version})
...
def menus(context, _get_config=None):
    ...
    #     if app_config.name.startswith(settings.APP_NAME)
    # ]
    ...
    # 当前用户使用的用户组(Cookie)
    group_id = int(context.request.COOKIES.get('cgid') or 0)

    # 菜单栏权限控制
    # (2): SIMPLEUI_CONFIG 的菜单配置弃用，只管理 MenuOwnerPermissionModel 中授予的菜单权限
    user_id = context.request.user.id
    config = GroupModel.get_menu_permissions(user_id=user_id, group_id=group_id)
    ...
    # 计算用户组权限
    groups = GroupModel.get_group_list(user_id=user_id, first_id=group_id)
...
def get_imd_tag(*context):
    print(context)
    return """<div id="ADDS">get_imd_tag()</div>"""
